...
## What it keeps, what it scrubs

The bias is asymmetric, on purpose:

- **Paths and IPs lean toward *under*-scrubbing.** System paths (`/usr`, `/etc`, …), file extensions, line numbers, private/loopback IPs, and diagnostic IDs (UUIDs, git SHAs) survive untouched — over-scrubbing destroys the trace's usefulness. A home path collapses to `~` but its relative tail lives on: `/Users/dpep/code/proj/src/db.rs:42` → `~/code/proj/src/db.rs:42`.
